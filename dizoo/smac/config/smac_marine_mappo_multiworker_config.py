...
main_config = dict(
    exp_name='smac_marine_mappo_seed0',
    env=dict(
        map_name='10m_vs_11m',
        difficulty=7,
...
if __name__ == '__main__':
    ...
    from ding.entry import serial_pipeline_marine_onpolicy_multiworker
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--seed', '-s', type=int, default=0)
    args = parser.parse_args()
    seed = args.seed
    main_config.exp_name = 'smac_marine_mappo_multiworker_seed' + str(seed)
    serial_pipeline_marine_onpolicy_multiworker((main_config, create_config), seed=seed, max_env_step=int(1e7))